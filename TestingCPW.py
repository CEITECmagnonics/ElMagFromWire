...
r_vals = np.linspace(1e-6, 100000e-6, 100)  # from 1 µm to 0.1 m
...
for i,f in enumerate(freqs):
    ...
    w = 2e-6                   # Width of the CPW (m)
    s = 2e-6                   # Gap between the CPW lines (m)
    ...
    A_z[:,i] = (mu0 * I0 / 4) * hankel2(0, kEff * r_vals) - (mu0 * I0 / 8) * hankel2(0, kEff * (r_vals-(s + w)))- (mu0 * I0 / 8) * hankel2(0, kEff * (r_vals+(s + w)))
    E_z[:,i]  = -1j*omega * A_z[:,i] 
    E_z[:,i]  = -1j*omega * A_z[:,i] 
    H_z[:,i] = (I0 * kEff /4) * hankel2(1, kEff * r_vals) - (I0* kEff /8) * hankel2(1, kEff * (r_vals-(s + w))) - (I0* kEff /8) * hankel2(1, kEff * (r_vals+(s + w)))
