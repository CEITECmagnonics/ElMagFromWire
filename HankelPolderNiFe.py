...
# =============================================================================
freqs = np.linspace(1e9, 12e9, 501)  # Frequency range from 1 GHz to 20 GHz
# freqs = [4.5e9]
...
for i,f in enumerate(freqs):
    ...
    epsilonR = 1 - 1j*1e7     # Relative permittivity (dimensionless) - Metal
    ...
    gamma = 30.4e9*2* np.pi    # Gyromagnetic ratio (rad/s/T)
    Msat = 747e3                 # Saturation magnetization (A/m)
    Bext = 7.2e-3                # External magnetic field (T)
    alpha = 70e-4                 # Gilbert damping factor
    K1 = 0                   #J/m3 anisotropy constant, assuming out-of-plane PMA
    omegaH = gamma * Bext
    ...
    muEff = (muEffxx*muEffzz + muEffxz**2)/muEffzz  # Use the zz component for effective permeability  
    muEff = muEffzz
    # =============================================================================
    # Calculate the effective wavenumber
    # =============================================================================
    kEff = omega * np.sqrt(muEff * epsilon) #Effective wavenumber
    kEff = np.where(np.imag(kEff) <= 0, kEff, -kEff)
    kEffArray[i] = kEff
...
axs_mag.set_ylim(1, Msat)  # Set y-axis limit for magnetization

axs_angle= fig.axes[4]
axs_angle.plot(r_vals * 1e6, np.rad2deg(np.arctan2(np.abs(Mz),Msat)))
...
vg1 = np.gradient(2 * np.pi * freqs, np.real(kEffArray))  # rad/s divided by rad/m gives m/s
...
expKreal = np.real(kEffArray) #Prepare for export
expKimag = np.imag(kEffArray) #Prepare for export
